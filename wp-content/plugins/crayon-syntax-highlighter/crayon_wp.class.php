...
class CrayonWP {
    ...
    public static function capture_crayons($wp_id, $wp_content, $extra_settings = array(), $args = array()) {
		...
		// escape crayon to ncrayonn
		$wp_content = preg_replace('#(crayon)#msi', 'n\0n', $wp_content);
        ...
        if ((CrayonGlobalSettings::val(CrayonSettings::CAPTURE_MINI_TAG) || $skip_setting_check) && $in_flag[CrayonSettings::CAPTURE_MINI_TAG]) {
            $wp_content = preg_replace('#(?<!\$)\[\s*(' . self::$alias_regex . ')\](.*?)\[\s*/\s*(?:\1)\s*\](?!\$)#msi', '[crayon lang="\1"]\2[/crayon]', $wp_content);
            $wp_content = preg_replace('#(?<!\$)\[\s*(' . self::$alias_regex . ')\b([^\]]*)/\s*\](?!\$)#msi', '[crayon lang="\1" \2 /]', $wp_content);
        }

        // Capture Markdown tags, currently only support ``` code ```
        if ((CrayonGlobalSettings::val(CrayonSettings::CAPTURE_MINI_TAG) || $skip_setting_check) && $in_flag[CrayonSettings::CAPTURE_MINI_TAG]) {
			$wp_content = preg_replace('#^```\s+(' . self::$alias_regex . ')(\s.*?)^```#msi', '[crayon lang="\1"] \2[/crayon]', $wp_content);
        }
		...
		// escape ncrayonn to crayon
		$wp_content = preg_replace('#n(crayon)n#msi', '\1', $wp_content);
		...
		// escape ncrayonn to crayon
		foreach($capture['capture'] as $key => $val) 
		{
			$capture['capture'][$key]['code'] = preg_replace('#n(crayon)n#msi', '\1', $capture['capture'][$key]['code']);
		}
